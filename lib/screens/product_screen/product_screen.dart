...
class ProductScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    _onChangedCategory(dynamic category) {
      ...
      bloc.add(ChangeProductInfoEvent(category: cat.name));
    }

    _onChangedUnit(dynamic unit) {
      var _unit = unit as ProductUnit;
      bloc.add(ChangeProductInfoEvent(unit: _unit.name));
    }

    _onChangedPrice(String price) {
      var _price = price.replaceAll(".", ",");
      _price = price.replaceAll("R\$", "");
      bloc.add(ChangeProductInfoEvent(price: _price));
    }

    _onChangedIsOrganic(bool isOrganic) {
      bloc.add(ChangeProductInfoEvent(isOrganic: isOrganic));
    }

    _onChangedHarvestDate(DateTime? date) {
      if (date != null) {
        var _date = formatter.format(date);
        bloc.add(ChangeProductInfoEvent(harvestDate: _date));
      }
    }

    _onChangedAvailableQuantity(String value){
      bloc.add(ChangeProductInfoEvent(availableQuantity: int.parse(value)));
    }
    ...
    _onChangedPicture(File? picture, int index) {
      if (picture != null) {
        ...
        bloc.add(
            ChangeProductInfoEvent(pictures: pictures, picturesMetadata: metadata));
      }
    }
                          ...
                          itemBuilder: (context, index) {
                            return Center(
                              child: PictureCard(
                                ...
                                picture:  index < bloc.product.pictures.length ? bloc.product.pictures[index] : null ,
                              ),
                            );
                          },
                        ...
                        children: [
                          CustomTextField(
                            ...
                            onChanged: (String name) =>
                                bloc.add(ChangeProductInfoEvent(name: name)),
                          ),
                          const SizedBox(
                            height: 10,
                          ),
                          CustomTextField(
                            ...
                            onChanged: (String desc) => bloc
                                .add(ChangeProductInfoEvent(description: desc)),
                          ),
                          const SizedBox(
                            height: 10,
                          ),
